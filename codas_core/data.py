...
def _short_source_name(name: str) -> str:
    base = Path(str(name or "dataset")).stem
    safe = "".join(ch if ch.isalnum() else "_" for ch in base).strip("_").lower()
    return (safe or "dataset")[:24]


def _detect_join_key(frames: list[pd.DataFrame]) -> str | None:
    """Pick an ID-like column present in *every* frame, by priority then by name pattern."""
    common = set(frames[0].columns)
    for frame in frames[1:]:
        common &= set(frame.columns)
    if not common:
        return None
    lowered = {str(col).lower(): col for col in common}
    for key in _ID_KEY_PRIORITY:
        if key in lowered:
            return lowered[key]
    for low, original in lowered.items():
        if low.endswith("_id") or low == "id" or "participant" in low or "subject" in low:
            return original
    return None


def merge_datasets(named_frames: list[tuple[str, pd.DataFrame]]) -> tuple[pd.DataFrame, dict]:
    """Combine several uploaded CSVs into one analysis frame so the agent can reason across
    them jointly.

    Strategy:
      * **join** on a shared ID column (e.g. participant_id) when one exists in every file —
        the common case of "features in one file, label columns in another".
      * **union** (stack rows) when the files share an identical schema but no ID key.
      * otherwise raise InsufficientDataError so the caller can explain the boundary rather
        than silently producing a meaningless cross-product.

    Colliding non-key column names are prefixed with the source's short name so provenance is
    preserved and nothing is silently overwritten.
    """
    named_frames = [(name, frame) for name, frame in named_frames if frame is not None and len(frame.columns)]
    if len(named_frames) < 2:
        raise InsufficientDataError("merge_datasets needs at least two datasets.")
    names = [_short_source_name(name) for name, _ in named_frames]
    # Disambiguate duplicate source names so prefixes stay unique.
    seen: dict[str, int] = {}
    for index, name in enumerate(names):
        if name in seen:
            seen[name] += 1
            names[index] = f"{name}_{seen[name]}"
        else:
            seen[name] = 0
    frames = [frame for _, frame in named_frames]
    sources = [
        {"name": names[i], "rows": int(len(frames[i])), "columns": int(frames[i].shape[1])}
        for i in range(len(frames))
    ]

    key = _detect_join_key(frames)
    if key is not None:
        # Prefix only the columns that collide across files (keep clean names otherwise).
        non_key_counts: dict[str, int] = {}
        for frame in frames:
            for col in frame.columns:
                if col != key:
                    non_key_counts[col] = non_key_counts.get(col, 0) + 1
        colliding = {col for col, count in non_key_counts.items() if count > 1}
        renamed: dict[str, str] = {}
        prepared = []
        for source_name, frame in zip(names, frames):
            frame = frame.copy()
            frame[key] = frame[key].astype(str)
            rename_map = {}
            for col in frame.columns:
                if col != key and col in colliding:
                    new_col = f"{source_name}_{col}"
                    rename_map[col] = new_col
                    renamed[f"{source_name}.{col}"] = new_col
            if rename_map:
                frame = frame.rename(columns=rename_map)
            prepared.append(frame)
        # Cardinality / row-explosion detection: a one-to-many or many-to-many join REPLICATES
        # rows from the smaller file(s), so the merged rows are NOT independent observations.
        # Surfacing this prevents silent pseudo-replication (inflated n / significance).
        per_source_card = []
        sources_with_dupe_keys = 0
        for src_name, frame in zip(names, prepared):
            vc = frame[key].value_counts()
            max_per_key = int(vc.max()) if len(vc) else 0
            per_source_card.append({
                "name": src_name, "rows": int(len(frame)), "unique_keys": int(frame[key].nunique()),
                "max_rows_per_key": max_per_key, "duplicate_key_rate": round(float((vc > 1).mean()) if len(vc) else 0.0, 3),
            })
            if max_per_key > 1:
                sources_with_dupe_keys += 1
        merged = prepared[0]
        for frame in prepared[1:]:
            merged = merged.merge(frame, on=key, how="outer")
        rows_before = sum(int(len(f)) for f in prepared)
        largest = max((int(len(f)) for f in prepared), default=0)
        rows_after = int(len(merged))
        cardinality = "many_to_many" if sources_with_dupe_keys >= 2 else ("one_to_many" if sources_with_dupe_keys == 1 else "one_to_one")
        key_sets = [set(f[key].dropna().unique()) for f in prepared]
        common = set.intersection(*key_sets) if key_sets else set()
        all_keys = set().union(*key_sets) if key_sets else set()
        unmatched_keys = int(len(all_keys) - len(common))
        explosion_warning = None
        if cardinality != "one_to_one":
            explosion_warning = (
                f"{cardinality.replace('_', '-')} join on '{key}': rows from the smaller file(s) are REPLICATED "
                f"across matches (merged to {rows_after:,} rows from sources of "
                f"{', '.join(str(c['rows']) for c in per_source_card)}). These {rows_after:,} rows are NOT "
                f"independent observations — treat '{key}' as a participant/cluster id (mixed-effects or "
                f"participant-level split), otherwise per-row significance is inflated."
            )
        info = {
            "strategy": "join",
            "join_key": key,
            ...
            "row_multiplier": round(rows_after / largest, 2) if largest else None,
            "per_source_key_cardinality": per_source_card,
            "unmatched_keys": unmatched_keys,
            "explosion_warning": explosion_warning,
        }
        return merged, info

    # No shared key: only safe to union if every file has the same column set.
    column_sets = [tuple(frame.columns) for frame in frames]
    if len(set(column_sets)) == 1:
        stacked = pd.concat(
            [frame.assign(__source=name) for name, frame in zip(names, frames)],
            ignore_index=True,
        )
        info = {
            "strategy": "union",
            "join_key": None,
            "sources": sources,
            "rows": int(len(stacked)),
            "columns": int(stacked.shape[1]),
            "renamed_columns": {},
        }
        return stacked, info

    raise InsufficientDataError(
        "These datasets don't share an ID column (e.g. participant_id) to join on, and their "
        "columns differ, so they can't be combined automatically. Give them a common ID column, "
        "or analyze one dataset at a time."
    )
